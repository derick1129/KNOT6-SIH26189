...
function EntitySearchBox({ label, onPick }: { label: string; onPick: (e: Entity) => void }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      api.get("/entities/search", { params: { q: query } }).then((res) => setResults(res.data));
    }, 250);
...
export default function PathFinder() {
  ...
  const run = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get("/graph/path", { params: { source: source.id, target: target.id } });
      setResult(res.data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card flex gap-4 items-end flex-wrap">
        <EntitySearchBox label="From" onPick={setSource} />
        <EntitySearchBox label="To" onPick={setTarget} />
        <button
